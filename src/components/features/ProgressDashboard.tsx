...
const ActivityHeatmap: React.FC<{ sessionHistory: { date: string; quizzesAttempted: number }[] }> = ({ sessionHistory }) => {
  ...
  return (
    <div>
      <h4 className="text-[10px] uppercase tracking-[0.2em] text-white/30 mb-2">Activity (30 days)</h4>
      <div
      ...
      >
        {cells.map((cell, i) => {
          ...
          const bg = cell.count === 0
            ...
            : `rgba(242,232,220,${0.15 + intensity * 0.55})`;
          return (
            <motion.div
              key={cell.date}
...
const TierProgressRings: React.FC<{ conceptsByTier: Record<number, ConceptNode[]>; getMastery: (id: string) => number }> = ({ conceptsByTier, getMastery }) => {
  ...
  const tierOpacities = [0.30, 0.40, 0.45, 0.55, 0.65];
  ...
  const rings = ([1, 2, 3, 4, 5] as Tier[]).map((tier, i) => {
    ...
    const cfg = TIER_CONFIG[tier];
    const opacity = tierOpacities[i];
    return { tier, radius, circumference, pct, label: cfg.label, mastered, total: concepts.length, opacity };
  });

  return (
    <div className="flex flex-col items-center">
      <h4 className="text-[10px] uppercase tracking-[0.2em] text-white/30 mb-3">Tier Mastery</h4>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        {rings.map((r, i) => (
          <g key={r.tier}>
            {/* background track */}
            <circle cx={cx} cy={cy} r={r.radius} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth={8} />
            {/* animated progress arc */}
            <motion.circle
              cx={cx} cy={cy} r={r.radius}
              fill="none"
              stroke={`rgba(255,255,255,${r.opacity})`}
              strokeWidth={8}
              strokeLinecap="round"
      ...
      <div className="flex flex-wrap gap-3 mt-2 justify-center">
        {rings.map(r => (
          <span key={r.tier} className="text-[10px] flex items-center gap-1">
            <span className="inline-block w-2 h-2 rounded-full bg-white" style={{ opacity: r.opacity }} />
            <span className="text-white/40">{r.label}</span>
            <span className="text-white/30">{r.mastered}/{r.total}</span>
          </span>
        ))}
      </div>
    </div>
...
const ConceptList: React.FC<{ title: string; items: { name: string; mastery: number }[]; variant: 'strengths' | 'review' }> = ({ title, items }) => {
  if (items.length === 0) return null;

  return (
    <div>
      <h4 className="text-[10px] uppercase tracking-[0.2em] text-white/30 mb-2">{title}</h4>
      <div className="space-y-1.5">
        {items.map((item, i) => (
          <motion.div
            key={item.name}
          ...
          >
            ...
            <span className="text-xs font-light px-2 py-0.5 rounded-full bg-white/[0.06] text-white/40">
...
const SessionStats: React.FC<{ currentSessionStart: number; profile: SessionStatsProfile }> = ({ currentSessionStart, profile }) => {
  ...
  return (
    <div>
      <h4 className="text-[10px] uppercase tracking-[0.2em] text-white/30 mb-2">Session Stats</h4>
...
const StatBox: React.FC<{ label: string; value: string; subtitle?: string }> = ({ label, value, subtitle }) => (
  <motion.div
    className="px-3 py-2.5 bg-white/[0.06] backdrop-blur-[12px] border border-white/[0.08] rounded-2xl text-center"
    initial={{ opacity: 0, y: 8 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.4 }}
  >
    <div className="text-xl font-light text-white/85">{value}</div>
    <div className="text-[10px] uppercase tracking-[0.2em] text-white/30">{label}</div>
    {subtitle && <div className="text-[9px] text-white/20">{subtitle}</div>}
  </motion.div>
);
...
export const ProgressDashboard: React.FC<ProgressDashboardProps> = ({ onClose, fullPage = false }) => {
  ...
  const content = (
    ...
    >
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-lg text-white/85 font-light tracking-[0.05em]">Progress Dashboard</h2>
        {onClose && (
          <button
            onClick={onClose}
            className="p-1.5 text-white/25 hover:text-white/60 rounded-lg hover:bg-white/5 transition-colors"
            aria-label="Close"
          >
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex justify-end" onClick={onClose}>
      <motion.div
        className="h-full w-full max-w-md bg-[#0A0707]/95 backdrop-blur-xl border-l border-white/[0.06] overflow-y-auto"
